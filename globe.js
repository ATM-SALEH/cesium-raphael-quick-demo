...
(function() {
    ...
    var HealthAndWealthDataSource = function() {
        ...
        this._wealthScale = function(x){ return 1000000};
        this._healthScale = function(x){ return 4000000};
        this._populationScale = function(x){ return 8};
        this._colorScale = function(x){ return "#ff7f0e"};
        ...
        function (movement) {
            ...
            if (Cesium.defined(pickedObject) && Cesium.defined(pickedObject.id)) {
                if (Cesium.defined(pickedObject.id.nationData)) {
                    //sharedObject.dispatch.on("nationMouseover", pickedObject.id.nationData, pickedObject);
                    $(sharedObject).trigger("nationMouseover",[pickedObject.id.nationData]);
    ...
    $(sharedObject).on("nationMouseover", function(event, nationObject) {
        ...
        var nationPopulation = (parseInt(nationObject.population) / 10);
        var nationIncome = (parseInt(nationObject.income) / 10)
        var nationLifeExpectancy = parseInt(nationObject.lifeExpectancy);

        var total = nationPopulation +  + nationIncome + nationLifeExpectancy;
        var population = (nationPopulation/ total) * 100;
        var lifeExpectancy = ((nationLifeExpectancy) / total) * 100;
        var income = (nationIncome / total) * 100;

        $("#chart").empty();
        var r = Raphael("chart"),
            pie = r.piechart(
                320, 180, 100,
                [population, income, lifeExpectancy],
                {
                    legend: ["Population", "Income", "Life Expectancy"],
                    legendpos: "north",
                    legendcolor: '#fff'
                });

        r.text(150, 20, nationObject.name).attr({ font: "24px sans-serif", fill: "#FFFFFF" });
